...
function entry(mediaPath, projectItem, projectItemId = '') {
  return { mediaPath, projectItem, projectItemId };
}
...
test('the same ProjectItem used on multiple tracks is returned once without a conflict', () => {
  ...
  const result = deduplicate([
    entry('C:\\Captures\\take.wav', projectItem, 'item-1'),
    entry('C:\\Captures\\take.wav', { wrapper: 'second' }, 'item-1'),
  ]);
...
test('path deduplication treats Windows slash and case variants as one media path', () => {
  ...
  const result = deduplicate([
    entry('D:\\VO\\voice-01.wav', projectItem),
    entry('d:/vo/VOICE-01.wav', projectItem),
    entry('D:\\VO\\voice-01.wav', projectItem),
  ]);
