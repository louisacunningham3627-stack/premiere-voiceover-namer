...
插件在录音停止、Premiere 生成 WAV 后等待文件稳定，再把它移入与当前 \`.prproj\` 同级的“录音”文件夹，命名为“项目名-32位UUID.wav”，并同步 Premiere 素材名、媒体路径和时间线片段名。跨卷时会排他复制并分块核对 SHA-256，链接成功后才清理原始采集文件；任何不确定情况都会保留源文件并提示。它不会在每条录音后保存整个 \`.prproj\`。macOS 外置卷权限、跨卷复制、真实录音、重链接和长时间连续录音仍需按项目验收清单验证。
`;
