...
Premiere Pro UXP 插件，将时间线录音在录制完成后自动移入 `.prproj` 同级的 `Adobe Premiere Pro Captured and Generated` 文件夹，并整理为：
...
插件会同步修改磁盘文件名、Premiere ProjectItem 素材名、当前序列中的时间线片段名和媒体路径。当前构建版本是 `0.4.0`，目标 Premiere Pro `25.6+`，开发机版本为 `26.0.1`。新增 Windows 回收助手已完成合成文件实测，Premiere 内联动仍待临时工程验收。

## 当前能力

- 自动回收的引用检查仅限当前工程；跨工程共用已登记录音时请暂停自动处理。回收后的 Premiere 撤销不能还原磁盘文件，需从系统回收站恢复。
- Windows 安装脚本把命名插件与按需启动的系统回收助手一起安装。仅新录制并签名登记的录音，在最后一个时间线引用消失至少 30 秒、全部序列检查通过后才回收；普通素材、历史录音不自动纳入。网络盘、可移动卷和不确定情况保留文件；Mac 暂不启用回收。详见 [回收机制与限制](docs/录音响应与回收开发状态.md)。
- 打开面板后，已保存工程且有活动序列时会自动建立监听并把当前序列既有素材记作基线；不需要选择目录，也不用点击“开始监听”。
- 用户只需点击时间线音轨上的 Premiere 原生“画外音录制”麦克风。录音停止后，插件会识别新素材并自动处理。
- 最终保存位置固定为 `<.prproj 所在目录>\Adobe Premiere Pro Captured and Generated`；插件会自动创建该文件夹。面板中的“备用源”只补充原始录音的发现范围，不能改变最终保存位置。
...
- 命名同时使用音轨变化事件、导入完成事件和 `1.2s` 目录差异轮询；没有新 WAV 时降低命名扫描频率。回收仅在空闲且有签名登记录音时约每 10 秒检查全项目引用，不依赖 Computer Use。
...
- 每条录音热路径只执行发现 WAV、等待稳定、生成 ID、同盘移动或跨盘排他复制、内容校验、单个 ProjectItem 重链接，以及当前序列全部对应 TrackItem 的名称同步和轻量 sidecar 更新；不调用 `project.save()`，也不重写整个 `.prproj`。
- 跨盘时，Premiere 可以先在 C 盘生成默认 WAV；插件以不覆盖模式复制到工程目录，用分块 SHA-256 核对源和目标内容，重链接成功后才清理原始采集文件。清理失败会保留原文件并给出中文警告，不回滚已经验证成功的工程目录链接。
- 只有已经位于当前工程 `Adobe Premiere Pro Captured and Generated` 文件夹中的 UUID 录音才走轻量名称修复。旧版留在其他目录的 UUID 录音可通过“扫描遗漏”移入，并保留原 UUID。
- 在工程旁写入 `<工程名>.voiceover-namer.json`，schema 4 只保存备用采集源等设置和最近处理记录，不保存也不依赖全局计数器。
- 只要求唯一命名时，协作者应使用 `0.3.2+`；要让最终文件统一进入工程旁的 `Adobe Premiere Pro Captured and Generated` 文件夹，所有继续录音的人都应使用 `0.3.4+`。
- `0.3.3` 曾错误使用工程旁的 `录音` 文件夹。升级后不会自动移动或删除该旧文件夹及其中素材；需要整理时先将旧文件夹设为“备用源”，再使用“扫描遗漏”确认处理。
- 接手人可以直接继续录音，不复制或合并计数器。若接手人使用旧版计数命名，新格式不会被自动改写；使用“扫描遗漏”确认后才会升级为 UUID。若两份交接材料出现相同 UUID：内容相同则视为同一素材，内容不同则必须隔离其中一份、生成新 UUID 并只重链接对应工程，禁止覆盖。
...
- 四步处理链直接跟随发现 WAV、等待写入稳定、移入工程并命名，以及 Premiere 重链接并同步时间线片段名的真实事务阶段。

## 面板怎么读

1. 打开并保存 Premiere 工程，打开要录音的活动序列；面板会自动待命。
2. 直接点击时间线音轨上的原生“画外音录制”麦克风并录音，停止后等待约 `3-4` 秒的文件稳定观察。
3. 插件按 `原始 WAV → 工程旁 Adobe Premiere Pro Captured and Generated 文件夹中的项目唯一名 → 自动重链接并同步片段名` 处理，面板会显示发现、稳定、移入命名和链接/片段同步阶段。
4. “备用源”只在自动识别原始录音失败时使用；它不是开始监听的前置条件，也不会修改最终保存位置。
...
默认地址为 `http://127.0.0.1:4174/?state=ready`。`state` 支持 `disconnected`、`unsaved`、`no-sequence`、`ready`、`starting`、`listening`、`paused`、`scanning`、`processing`、`loading` 和 `error`；处理与错误状态还可用 `stage=found|stable|rename|relink` 检查各阶段。预览直接复用生产版状态推导器，供人工检查布局与文字溢出，不能替代 UXP 或 Premiere 实机验收；端口被占用时可先设置 `PANEL_PREVIEW_PORT`。
...
开发机 Premiere Pro `26.0.1` 的 UXP 启动日志已确认插件 ID 可从用户级 fallback 目录被宿主发现。安装或更新 `0.3.4` 前必须先正常关闭 Premiere，完成后重新启动一次；连续录音期间不需要重启。真实录音、移动、内容校验、重链接与时间线片段名同步仍须按 [运行验收清单](docs/运行验收清单.md) 在临时工程完成，全部通过前保持 `runtime_tested=false`。
...
卸载不会直接删除插件，只会移动到 `~/Library/Application Support/Adobe/UXP/PluginBackups`。Mac 版本已覆盖 POSIX 路径、根目录和大小写敏感规则；macOS Premiere 的真实录音、外置卷权限、跨卷复制、重链接和连续录音仍须按 [运行验收清单](docs/运行验收清单.md) 完成实机验收。
...
- 同盘使用文件系统移动；跨盘使用 `COPYFILE_EXCL` 排他复制，复制返回成功且源/目标 SHA-256 一致后才重链接。源清理前会再次核对当前项目、目标副本和源文件身份；任何不确定情况都保留原始数据。
...
- 在正常保存前发生崩溃或强制退出时，录音可能已经移动或复制到工程旁的 `Adobe Premiere Pro Captured and Generated` 文件夹，但上次工程快照仍保存原始采集路径；重开后可能看到路径过时或素材离线。此时先按目录扫描/重新链接流程恢复，未来可增加恢复 journal 提高崩溃恢复能力。
