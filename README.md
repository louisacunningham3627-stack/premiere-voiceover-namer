...
插件会同步修改磁盘文件名、Premiere ProjectItem 素材名和媒体路径。当前构建版本是 `0.3.0`，目标 Premiere Pro `25.6+`，开发机版本为 `26.0.1`。
...
- 等待文件大小和修改时间连续稳定后再改名，遇到 Windows 文件锁会有限重试。
...
- 每条录音热路径只执行发现 WAV、等待稳定、生成 ID、磁盘改名、单个 ProjectItem 重链接/素材名同步和轻量 sidecar 更新，不调用 `project.save()`，也不重写整个 `.prproj`。
- 在工程旁写入 `<工程名>.voiceover-namer.json`，schema 4 只保存设置和最近处理记录，不保存也不依赖全局计数器。
- 会继续录音的每位协作者都应侧载 `0.3.0+` 或兼容该 UUID 规则的版本；复制旧版 sidecar 不能让旧计数命名获得跨副本唯一性。
...
- 四步处理链直接跟随发现 WAV、等待写入稳定、磁盘改名和 Premiere 单素材重链接的真实事务阶段。

## 面板怎么读

1. 打开并保存 Premiere 工程，打开要录音的活动序列；面板会自动待命。
2. 直接点击时间线音轨上的原生“画外音录制”麦克风并录音，停止后等待约 `3-4` 秒的文件稳定观察。
3. 插件按 `WAV → 项目唯一名 → 自动重链接` 处理，面板会显示发现、稳定、改名和重链接阶段。
...
旧版本会先移动到 `%APPDATA%\Adobe\UXP\PluginBackups`，安装失败时自动恢复。安装或更新后只需重新启动一次 Premiere，再打开 `Window > UXP Plugins > 赫朝录音命名器`；连续录音期间不需要重启。协作者可在自己的 Windows 账户运行同一命令，不共享本机状态或计数器。
...
开发机 Premiere Pro `26.0.1` 的 UXP 启动日志已确认插件 ID 可从用户级 fallback 目录被宿主发现。安装或更新 `0.3.0` 前必须先正常关闭 Premiere，完成后重新启动一次；连续录音期间不需要重启。真实录音、改名与重链接仍须按 [运行验收清单](docs/运行验收清单.md) 在临时工程完成，全部通过前保持 `runtime_tested=false`。
...
4. 在 Premiere 中打开 `Window > UXP Plugins > 赫朝录音命名器`。
...
- 同一 Premiere 素材即使出现在多个音轨也只处理一次；只有同一磁盘路径关联到多个不同 ProjectItem 时才会保守跳过并记录错误。
