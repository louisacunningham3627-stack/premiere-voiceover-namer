...
Premiere Pro UXP 插件，将时间线录音在录制完成后自动移入 `.prproj` 同级的 `录音` 文件夹，并整理为：
...
插件会同步修改磁盘文件名、Premiere ProjectItem 素材名、当前序列中的时间线片段名和媒体路径。当前构建版本是 `0.3.3`，目标 Premiere Pro `25.6+`，开发机版本为 `26.0.1`。

## 当前能力

- 打开面板后，已保存工程且有活动序列时会自动建立监听并把当前序列既有素材记作基线；不需要选择目录，也不用点击“开始监听”。
- 用户只需点击时间线音轨上的 Premiere 原生“画外音录制”麦克风。录音停止后，插件会识别新素材并自动处理。
- 最终保存位置固定为 `<.prproj 所在目录>\录音`；插件会自动创建该文件夹。面板中的“备用源”只补充原始录音的发现范围，不能改变最终保存位置。
...
- 只有已经位于当前工程 `录音` 文件夹中的 UUID 录音才走轻量名称修复。旧版留在其他目录的 UUID 录音可通过“扫描遗漏”移入，并保留原 UUID。
- 在工程旁写入 `<工程名>.voiceover-namer.json`，schema 4 只保存备用采集源等设置和最近处理记录，不保存也不依赖全局计数器。
- 只要求唯一命名时，协作者应使用 `0.3.2+`；要让最终文件统一进入工程旁的 `录音` 文件夹，所有继续录音的人都应使用 `0.3.3+`。
...
3. 插件按 `原始 WAV → 工程旁“录音”文件夹中的项目唯一名 → 自动重链接并同步片段名` 处理，面板会显示发现、稳定、移入命名和链接/片段同步阶段。
...
开发机 Premiere Pro `26.0.1` 的 UXP 启动日志已确认插件 ID 可从用户级 fallback 目录被宿主发现。安装或更新 `0.3.3` 前必须先正常关闭 Premiere，完成后重新启动一次；连续录音期间不需要重启。真实录音、移动、内容校验、重链接与时间线片段名同步仍须按 [运行验收清单](docs/运行验收清单.md) 在临时工程完成，全部通过前保持 `runtime_tested=false`。
...
- 在正常保存前发生崩溃或强制退出时，录音可能已经移动或复制到工程旁的 `录音` 文件夹，但上次工程快照仍保存原始采集路径；重开后可能看到路径过时或素材离线。此时先按目录扫描/重新链接流程恢复，未来可增加恢复 journal 提高崩溃恢复能力。
